...
# new update
	...
	def depreciation_in_kilo(self):
		...
		for roasted in roasted_list:
			average_depreciation = round(((float(roasted.raw)) - float(roasted.roasted))/(float(roasted.raw)+0.0001)*100,2)
...
class Roaster(models.Model):
	...
	def _get_depreciation(self):
		value = round(((float(self.raw)+0.0001) - float(self.roasted))/(float(self.raw)+0.00001)*100 ,2)
		return "{0}\t %".format(value)
...
# weight lose embeded to beansgudang
	...
	def roaster_product_name(self):
		return ('{0} RD:{1}/ SET{2} / {3}/{4}/{5}').format(self.beans_name, self.roast_date, self.set_number ,self.mesin, self.batch_number, self.shift)
...
class ForeignMaterialReport(models.Model):
	...
	def __str__(self):
		return(('{0}-{1}').format(self.created_date, self.masuk))
